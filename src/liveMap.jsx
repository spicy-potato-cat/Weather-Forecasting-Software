...
// Config: optimized for better visualization
const CONFIG = {
  // Particles
  DENSITY_PER_PIXEL: 0.0008, // Adjusted for better distribution
  COUNT_MIN: 800,
  COUNT_MAX: 5000,
  DOT_RADIUS_PX: 1.2,
  ...
  GRID_STEP_DEG: 1.5, // Finer grid for better interpolation
  PRIME_MAX_CELLS: 40,
...
const LiveMap = () => {
  ...
  const respawnParticle = (p) => {
    ...
    p.speed = 0; // Initialize speed for coloring
  };
  ...
  const sampleWindUV = async (lon, lat) => {
    ...
    // If all present, do bilinear interpolation on u, v, and speed separately
  ...
  const animate = async (ts) => {
    ...
    // Fade for smooth trails (exponential decay)
    const canvas = canvasRef.current;
    ...
    if (CONFIG.USE_SPEED_COLOR) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        ctx.fill(new Path2D(`M${px[0]},${px[1]} a${CONFIG.DOT_RADIUS_PX},${CONFIG.DOT_RADIUS_PX} 0 1,0 0,0.01`));
      }
    } else {
