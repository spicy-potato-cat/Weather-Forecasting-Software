...
// POST /api/auth/forgot-password (for non-authenticated users)
    ...
    try {
      ...
      await query(
        `INSERT INTO password_reset_tokens (user_id, token, expires_at)
         ...
         ON CONFLICT (user_id) 
         DO UPDATE SET token = $2, expires_at = NOW() + INTERVAL '10 minutes', created_at = NOW()`,
        [user.id, otp]
      );
      ...
      try {
        await emailService.sendEmail({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #2fe79f;">Password Reset Request</h2>
              <p>Hello ${user.name},</p>
              ...
              <p>If you didn't request this, please ignore this email.</p>
              <hr style="border: 1px solid #eee; margin: 20px 0;">
              <p style="color: #666; font-size: 12px;">Aether Weather</p>
            </div>
          `
        });
...
// POST /api/auth/reset-password (for non-authenticated users)
